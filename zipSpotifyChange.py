...
# Spotify 폴더의 경로를 지정하세요.
spotify_folder = "spotify"

# 병합된 데이터를 저장할 폴더와 파일 경로
output_folder = "json"
output_file = os.path.join(output_folder, "spotifyChange.json")
...
# 모든 JSON 파일을 처리
for country in os.listdir(spotify_folder):
    country_folder = os.path.join(spotify_folder, country)
                ...
                transformed_data = [
                    {
                        ...
                        "youtubeID": "",  # Spotify 데이터에 youtubeID가 없으므로 빈 값 설정
                        "spotifyID": item.get("spotifyID", ""),
                        "appleID": ""     # 추가 데이터가 없으므로 빈 값 설정
                    }
                    for item in data
                ]
